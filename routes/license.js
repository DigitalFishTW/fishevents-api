...
router.patch('/:id', function (req, res, next) {
    ...
    .then(function (patchedPermit) {
        patched = patchedPermit;
        dataObj = {
            meta: {}
        }
        var i;
        _id = patchedPermit._id;
        delete patchedPermit._id;
        
        for (i in patchedPermit) {
            if (0 <= mustHave.indexOf(i)) {
                dataObj[i] = patchedPermit[i];
            } else {
                dataObj.meta[i] = patchedPermit[i];
            }
        }
        return Permit.update({_id: _id}, dataObj).exec();
    })
    .then(function (doc) {
